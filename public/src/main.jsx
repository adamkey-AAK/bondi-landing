...
function App() {
  ...
  const [headerFog, setHeaderFog] = useState(0);

  useEffect(() => {
    const updateHeaderFog = () => {
      const progress = Math.min(window.scrollY / 96, 1);
      setHeaderFog(progress);
    };

    updateHeaderFog();
    window.addEventListener("scroll", updateHeaderFog, { passive: true });
    return () => window.removeEventListener("scroll", updateHeaderFog);
  }, []);
  ...
  return (
    <main>
      ...
      <div className="page">
        <header className="header" style={{ "--header-fog": headerFog }}>
          <BondiLogo />
          <div />
        </header>
          ...
          <div className="layout">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="form">
                ...
                <div
                  className={`success-card ${submitted ? "is-visible" : ""}`}
...
if (typeof window !== 'undefined') {
  const updateLogoFog = () => {
    const opacity = Math.min(window.scrollY / 90, 1);
    document.documentElement.style.setProperty('--logo-fog-opacity', opacity.toFixed(3));
  };
  updateLogoFog();
  window.addEventListener('scroll', updateLogoFog, { passive: true });
}
