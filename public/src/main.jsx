...
function BondiLogo({ fog = 0 }) {
  return (
    <motion.div
      className="bondi-logo-shell"
      style={{ "--logo-fog": fog }}
      initial={{ opacity: 0, y: -8 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.8 }}
    >
      <img className="bondi-logo" src="/bondi-logo.png" alt="Bondi" />
    </motion.div>
  );
}
...
function App() {
  ...
  const [logoFog, setLogoFog] = useState(0);

  useEffect(() => {
    const updateLogoFog = () => {
      setLogoFog(Math.min(window.scrollY / 90, 1));
    };

    updateLogoFog();
    window.addEventListener("scroll", updateLogoFog, { passive: true });
    return () => window.removeEventListener("scroll", updateLogoFog);
  }, []);
